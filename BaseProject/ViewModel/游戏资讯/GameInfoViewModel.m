...
- (void)getDataFromNetCompleteHandle:(CompletionHandle)completionHandle{
    self.dataTask = [GameInfoNetManager getGameInfoWithType:_type start:_start completionHandle:^(GameInfoModel *model, NSError *error) {
        if (_start == 0) {
...
- (GameInfoDataIndexpicModel *)modelForArr:(NSArray *)arr row:(NSInteger)row{
    return arr[row];
}
...
- (NSArray *)iconURLSForRowInList:(NSInteger)row{
    ...
    for (int i = 0; i < arr.count; i ++) {
        GameInfoDataIndexpicShowitemModel *model= arr[i];
