...
- (void)getDataFromNetCompleteHandle:(CompletionHandle)completionHandle{
    self.dataTask=[LOLNetManager getZBItemListWithTag:_tag completionHandle:^(id model, NSError *error) {
        if (!error) {
            self.dataArr = model;
        }
...
- (NSURL *)iconURLForRow:(NSInteger)row{
    return [NSURL URLWithString:[NSString stringWithFormat:@"http://img.lolbox.duowan.com/zb/%@_64x64.png", @([self modelForRow:row].ID).stringValue]];
}
