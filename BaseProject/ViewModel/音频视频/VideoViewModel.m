...
- (NSString *)playForRow:(NSInteger)row{
    return @([self videoListModelForRow:row].playCount).stringValue;
}
- (NSString *)timeForRow:(NSInteger)row{
    return [self videoListModelForRow:row].ptime;
}
- (void)getDataFromNetCompleteHandle:(CompletionHandle)completionHandle{
    self.dataTask = [MultimediaNetManager getVideoWithIndex:_index completionHandle:^(VideoModel *model, NSError *error) {
        if (_index == 0) {
