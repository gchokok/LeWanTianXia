...
@interface GameInfoPicViewController ()<MWPhotoBrowserDelegate>
@property(nonatomic,strong) GameInfoPicViewModel *gamePicVM;

@end

@implementation GameInfoPicViewController

- (GameInfoPicViewModel *)gamePicVM{
    if (!_gamePicVM) {
        _gamePicVM = [[GameInfoPicViewModel alloc] initWithAid:_aid];
    }
    return _gamePicVM;
}
...
- (void)viewDidLoad {
    ...
    //请求时要有正在操作的提示
    [self showProgress];
    [self.gamePicVM getDataFromNetCompleteHandle:^(NSError *error) {
        [self hideProgress];
...
- (NSUInteger)numberOfPhotosInPhotoBrowser:(MWPhotoBrowser *)photoBrowser{
    return self.gamePicVM.rowNumber;
}
- (id <MWPhoto>)photoBrowser:(MWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index{
    MWPhoto *photo = [MWPhoto photoWithURL:[self.gamePicVM picURLForRow:index]];
    return photo;
}
